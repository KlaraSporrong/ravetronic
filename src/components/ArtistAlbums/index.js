...
export default class ArtistAlbums extends Component {
          ...
          results.push(
            <div className="col-md-3">
              <Card
                ...
                onClick={event =>
                  this.getAlbumTracks(event, album.id, album.name)
                }
                text="Show Tracks"
              />
            </div>
  ...
  getAlbumTracks = (event, albumId, name) => {
    ...
    let cleanName = name
      ...
      .trim();
    axios
      .get(`${spotifyAlbumURL}${albumId}/tracks?access_token=${authToken}`)
      .then(response => {
        this.setState({ tracks: response.data.items });
        tracks = response.data.items;
      })
      .then(() =>
        this.props.history.push(`/album-tracks/${albumId}/${cleanName}`, {
          data: { tracks },
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="row">{this.showAlbums(albums)}</div>
      </div>
    );
  }
}
