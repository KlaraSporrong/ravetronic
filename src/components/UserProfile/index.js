...
export default class UserProfile extends Component {
  ...
  componentDidMount = () => {
    axios
      .get(`${spotifyPlaylistURL}${this.state.auth.authToken}`)
      .then(response => {
        this.setState({
    ...
    return (
      <div>
        <div className="row mt-5">
          <button onClick={this.routeBack} className="btn btn-outline-success">
            Back
          </button>
        </div>
        ...
        <div className="row mt-3">{this.renderPlaylist()}</div>
      </div>
    );
  }
}
