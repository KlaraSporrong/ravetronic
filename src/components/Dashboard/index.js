...
export default class Dashboard extends Component {
  ...
  searchSpotifyArtists = event => {
    ...
    axios
      .get(
        `${spotifySearchURL}${
          this.state.query
        }&type=artist&access_token=${authToken}`
      )
      .then(response => {
          ...
          results.push(
            <div className="col-md-3">
              <Card
                ...
                onClick={event =>
                  this.searchAlbums(event, artist.id, artist.name)
                }
                text="Show Albums"
              />
            </div>
  ...
  searchAlbums = (event, artistId, name) => {
    ...
    axios
      .get(
        `${spotifyArtistURL}${artistId}/albums?album_type=album&access_token=${authToken}`
      )
      .then(response => {
      ...
      .then(() =>
        this.props.history.push(`/artist-albums/${artistId}/${cleanName}`, {
          data: { albums },
  ...
  render() {
    const {
      images,
      display_name
    } = this.props.location.state.current_user.user;
    return (
      <div>
        <Nav
